...
from dbt.contracts.graph.unparsed import EntityDimensionType
...
class SetMeasureAggregationTimeDimension(ABC):
    ...
    def _find_primary_time_dimension(entity: Entity) -> Optional[TimeDimensionReference]:
        for dimension in entity.dimensions:
            if dimension.type == EntityDimensionType.TIME and dimension.type_params and dimension.type_params.is_primary:
                return dimension.time_dimension_reference
        return None

    @staticmethod
    def _transform_entity(entity: Entity) -> Entity:  # noqa: D
        primary_time_dimension_reference = SetMeasureAggregationTimeDimension._find_primary_time_dimension(
            entity=entity
        )
