...
from dbt.adapters.relation.models import DescribeRelationResults, Relation, RelationStub
...
class Materialization(ABC):
    ...
    def existing_relation(
        ...
        """
        Produce a full-blown `Relation` instance for `self.existing_relation_stub` using metadata from the database

        Args:
            describe_relation_results: the results from the macro `describe_sql(self.existing_relation_stub)`

        Returns:
            a `Relation` instance that represents `self.existing_relation_stub` in the database
        """
        if self.existing_relation_stub:
            relation_type = self.existing_relation_stub.type
    ...
    @property
    def backup_relation_stub(self) -> Optional[RelationStub]:
        if self.existing_relation_stub:
            return self.relation_factory.make_backup_stub(self.existing_relation_stub)
        return None
