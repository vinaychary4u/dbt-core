...
from tests.adapter.dbt.tests.adapter.materialized_view.base import Base, assert_message_in_logs
...
class OnConfigurationChangeBase(Base):
    ...
    def alter_message(self, project):
        return f"Applying ALTER to: {relation_from_name(project.adapter, self.base_materialized_view.name)}"

    @pytest.fixture(scope="function")
    def create_message(self, project):
        return f"Applying CREATE to: {relation_from_name(project.adapter, self.base_materialized_view.name)}"

    @pytest.fixture(scope="function")
    def refresh_message(self, project):
        return f"Applying REFRESH to: {relation_from_name(project.adapter, self.base_materialized_view.name)}"

    @pytest.fixture(scope="function")
    def replace_message(self, project):
        return f"Applying REPLACE to: {relation_from_name(project.adapter, self.base_materialized_view.name)}"
