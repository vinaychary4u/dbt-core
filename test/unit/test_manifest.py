    ...
    def setUp(self):
        ...
        self.nested_nodes = {
            ...
            "model.root.dep": ModelNode(
                ...
                sources=[],
                metrics=[],
                depends_on=ModelDependsOn(nodes=["model.root.events"]),
                config=self.model_config,
                tags=[],
                path="multi.sql",
            ...
            "model.root.nested": ModelNode(
                ...
                sources=[],
                metrics=[],
                depends_on=ModelDependsOn(nodes=["model.root.dep"]),
                config=self.model_config,
                tags=[],
                path="multi.sql",
                original_file_path="multi.sql",
            ...
            "model.root.multi": ModelNode(
                ...
                sources=[],
                metrics=[],
                depends_on=ModelDependsOn(nodes=["model.root.nested", "model.root.sibling"]),
                config=self.model_config,
                tags=[],
                path="multi.sql",
                original_file_path="multi.sql",
    ...
    def setUp(self):
        ...
        self.nested_nodes = {
            ...
            "model.root.dep_version": ModelNode(
                ...
                sources=[],
                depends_on=ModelDependsOn(nodes=["model.root.versioned.v1"]),
                config=self.model_config,
                tags=[],
