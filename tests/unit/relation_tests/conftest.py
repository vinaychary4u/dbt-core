...
def test_view_ref(view_ref):
    assert view_ref.name == "my_view"


def test_materialized_view_compiled_node(materialized_view_compiled_node):
    assert materialized_view_compiled_node.name == "my_materialized_view"
    assert materialized_view_compiled_node.config.full_refresh is True
    assert materialized_view_compiled_node.config.on_configuration_change == "continue"


def test_materialized_view_relation(materialized_view_relation):
    # covers `materialized_view_describe_relation_results` as a dependent fixture
