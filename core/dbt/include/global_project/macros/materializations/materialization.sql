{# /*
    These are step macros for building out a materialization based on `MaterializationConfigBase`, which can be
...
{% macro mat_setup(materialization_config, pre_hooks) %}
    ...
    {% set backup_relation = load_cached_relation_config_backup(materialization_config) %}
    {% set intermediate_relation = load_cached_relation_config_intermediate(materialization_config) %}
...
{% macro mat_execute_build_sql(materialization_config, build_sql, relation_exists, post_hooks) %}
    ...
    {% set should_revoke = should_revoke(relation_exists, full_refresh_mode=True) %}
