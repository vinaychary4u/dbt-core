...
from dbt.adapters.postgres.relation.models import PostgresSchemaRelation
...
def test_make_schema(config_dict: dict, exception: Type[Exception]):
    if exception:
        with pytest.raises(exception):
            PostgresSchemaRelation.from_dict(config_dict)
    else:
        my_schema = PostgresSchemaRelation.from_dict(config_dict)
